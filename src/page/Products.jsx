...
export default function Products() {
    ...
    return (
        <div className=' min-h-screen max-h-full  bg-white bg-fixed w-full' >
            <div className='h-20 pt-5 flex justify-between  pr-10 pl-10 bg-amber-400'>
                <div className='flex '>
                ...
                <div className='flex gap-8'>
                    <Link to=''><FaRegBell className='text-3xl text-white shadow-[0px_0px_30px_2px] shadow-black/10 p-2 h-10 w-10 rounded-2xl' /></Link>
                    <Link to='/profile'><CgProfile className='text-3xl text-white shadow-[0px_0px_30px_2px] shadow-black/10 p-2 h-10 w-10 rounded-2xl' /></Link>
                </div>
            </div>
            <div className='pb-10 pl-10 pr-10 pt-5'>
                ...
                {show ?
                    <div>
                        ...
                        <button className='float-right text-md text-black cursor-pointer pr-5 mt-5' onClick={() => setShow(!show)}>Show less</button>
                        <div className=' grid grid-cols-4 gap-15 mt-5 w-full'>
                            {state.map((cards, i) => (

                                <div onClick={() => handleclick(cards.id)} className='justify-items-center  shadow-[0px_20px_35px_10px] shadow-black/20 p-2 rounded-3xl text-black  backdrop-blur-2xl '>
                    ...
                    <div>
                        <h1 className='inline text-5xl ml-5 font-bold'>Cars</h1>
                        <button className='float-right text-md text-black cursor-pointer pr-5 mt-5' onClick={() => setShow(!show)}>Show all</button>
                        <div className=' grid grid-cols-4 gap-15 w-full mt-5'>
                            {state.slice(0, 4).map((cards, i) => (
                                <div onClick={() => handleclick(cards.id)} className='justify-items-center  shadow-[0px_15px_35px_5px] shadow-black/30 p-2 rounded-3xl text-black  backdrop-blur-2xl '>
                                    <img src={cards.image} className='h-50 w-full rounded-t-3xl ' alt="nothing" />
                                    <h1 className='float-start pt-3'>{cards.name}</h1>
                                    <div className='flex justify-between w-full pb-3'>
                                        <p className='flex'><IoLocationOutline className='relative top-1' />{cards.location}</p>
